...
const Order = ({ route }) => {
	...
	let OrderReducerModifiedData = ProductsOrderReducerRedux.map((obj) => {
		return { addons: obj?.Addons, count: obj?.Count, product: obj?.Product?.id }
	})

	console.log(OrderReducerModifiedData, "OrderReducerModifiedData")

	var orderDataForAPI = {
		cardId: cardSelected,
		products: OrderReducerModifiedData,
	}
